...
# Frees ports 8000 and 8080 first so "address already in use" is avoided.
# Run from project root: ./start_demo.sh

set -e
cd "$(dirname "$0")"

# Free port if something is already bound (Linux/WSL/macOS)
free_port() {
  local port=$1
...
echo "Freeing ports 8000 and 8080 if in use..."
free_port 8000
free_port 8080
