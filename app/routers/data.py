...
def get_crm_customers(
    ...
    customer_id: Optional[str] = Query(None, description="Filter by customer_id (overrides company when set)"),
    ...
    params = {
        "customer_id": customer_id or company_id,
        "top": top,
        "period": period,
    }
...
def get_support_tickets(
    company_id: str = Depends(get_company_id),
    customer_id: Optional[str] = Query(None, description="Filter by customer_id (default: your company)"),
    ...
    raw_data = connector.get_data(
        {"customer_id": customer_id or company_id, "status": status, "priority": priority}
    )
...
def get_analytics_metrics(
    ...
    raw_data = connector.get_data(
        {"customer_id": customer_id or company_id, "from": from_date, "to": to_date, "limit": limit}
    )
